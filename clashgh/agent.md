...
- Admin: new Schedule page (`#/schedule`, `GET /api/admin/schedule`) lists every unfinished match in live cups with state (no time yet / proposed / agreed / playing / awaiting results), overdue flag, window close, proposer and both players' opt-in contact numbers. Tournament detail bracket table and dispute cards show the agreed or proposed time and reschedule count. Read only by design: the scheduling rules enforce themselves.
- Android release: `mobile/eas.json` (preview APK, production AAB, submit to Play internal track), `app.json` gains versionCode, permissions, App Links intent filter for https://clashgh.app/app, splash, runtimeVersion. API serves `/.well-known/assetlinks.json` from `ANDROID_SHA256_FINGERPRINTS`. Store listing copy, Play declarations and the closed beta checklist are in `clashgh/RELEASE.md`.

## Addendum: web first launch, 2026-09-16

- Decision: launch on the web only (site at /, player app at /app in the phone browser, add to home screen). Android/Play Store comes after the web version earns. `RELEASE.md` is ordered that way.
- Paystack client corrected against the current API docs: `POST /transaction/initialize` with `channels: ['mobile_money']` and `callback_url` back to `/app/tournament/:id` (response has `authorization_url` at top level); `POST /transferrecipient` with `type: mobile_money`, local `0XXXXXXXXX` number, telco `bank_code` (MTN, VOD, ATL), GHS; `POST /transfer` with `recipient_code` and a `[a-z0-9_-]{16,50}` reference per attempt; `GET /transaction/verify/:reference` added. The pending TTL reaper in live mode verifies a stale pending charge with Paystack before releasing the slot (lost webhook safety net). `test/paystack-live.test.mjs` runs the live code path against an in-process mock and asserts the exact request shapes plus webhook signature and idempotency. `test/mock-paystack.js` updated to the same endpoints.
- Web join flow opens Paystack in the same tab (phones block popups); the player returns via callback_url to the tournament page, which shows the paid state or lets them resume a pending payment.
- render.yaml now declares the EXPO_PUBLIC_* build env so the exported web app is built in supabase/live mode on Render.
- Site copy: "Nothing to install" card, FAQ "Do I need to download an app?", about page line.
